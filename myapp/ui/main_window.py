...
class MainWindow(QMainWindow):
    def __init__(self, settings: dict):
        ...
        self.instructions_label = QLabel(
            "YOLO - Gestes: Main au centre = PLAY/PAUSE | Main droite = AVANCER | Main gauche = RECULER\n"
    ...
    def set_processor(self, mode: str):
        ...
        if mode == "yolo":
            try:
                ...
                processor = HandYolo(name="HandYolo", config=self.settings, **eng_cfg)
                ...
                self.log.info("YOLO initialisé avec succès")
            except Exception as e:
                self.log.exception("Init HandYolo échouée: %s", str(e))
                # Afficher un message d'erreur à l'utilisateur
                QMessageBox.warning(
                    self,
                    "Erreur YOLO",
                    f"Impossible d'initialiser YOLO:\n{str(e)}\n\n"
                    "Assurez-vous que ultralytics est installé:\n"
                ...
                self.btn_yolo.setChecked(False)
